...
def render():
    ...
    if years:
        ...
        with c1: st.metric("전기 등록 대수", f"{int(ev):,}대")
        with c2: st.metric("하이브리드 등록 대수", f"{int(hy):,}대")
        with c3: st.metric("차이(하이브리드-전기)", f"{int(diff):+,}대", delta=(f"{ratio:.1f}%" if ratio is not None else None))

    st.subheader("등록 현황 표")
    st.caption(f"선택 지역: {region}")
    st.dataframe(
        df_table.rename(columns={"reg_year": "등록 연도","fuel_type": "연료 유형","reg_count": "등록 대수"}),
